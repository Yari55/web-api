...
class OpenAIClient:
    def __init__(self, timeout=180, proxy=None):
        self.proxies = proxy
        self.timeout = timeout
        self._host = "https://sharegpt.new.oaifree.com"
        self.http_client = httpx.AsyncClient(
            base_url=self._host, proxies=self.proxies, timeout=self.timeout
        )
    ...
    async def __handle_route(self, route):
        ...
        try:
            ...
            logger.error(f"[OpenAIClient.__handle_route] Error occurred: {e}")
        finally:
            response_body = "".join([f"{line.decode() if isinstance(line, bytearray) else line}\n\n" for line in self.origin_response])
            if response is not None:
                response_body = "".join(
                    [f"{line.decode() if isinstance(line, bytearray) else line}\n\n" for line in self.origin_response]
                )
                await route.fulfill(
                ...
                await route.fulfill(
                    status=500,
                    body="Internal Server Error: Unable to process the request.",
                )
...
# 启动 OpenAIClient 并设置代理
client = OpenAIClient(timeout=180)
asyncio.run(client.setup_playwright())
